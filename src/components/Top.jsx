...
const Top = (props) => {
  ...
  return (
    <div
      className={` bg-no-repeat bg-cover  xs:bg-pink-bg max-xs:items-start max-sm:hidden bg-gradient-to-tr max-[420px]:hidden from-black to-gray-800  max-xs:bg-contain sm:h-[70vh] lg:h-[80vh] xl:h-[85vh] flex max-xs:h-fit items-center max-xs:p-1 xs:p-5 justify-start `}
    >
      <div className=" sm:w-[90%] lg:w-[60%]  max-xs:w-[100%] z-2 flex max-xs:h-[300px] xs:h-full max-xs:p-3 max-xs:justify-center items-start p-2 flex-col ">
        ...
        <Link to="/menu" >
          <button onClick={()=>{setActiveLink('menu')}} className="bg-white max-[375px]:font-medium max-xs:mt-4 hover:scale-95 max-xs:px-4 max-xs:py-1.5 max-xs:text-sm duration-200 px-6 py-2 rounded-xl sm:mt-4 lg:mt-5 xl:mt-8">
            Explore
          </button>
        </Link>
      </div>
      <div
        className={`coffee-bg  flex items-center sm:right-1 z-1 absolute lg:right-2 xl:right-10 max-lg:hidden  sm:h-[330px] sm:w-[400px] max-xs:justify-center justify-center xl:h-[30.8rem] xl:w-[600px] lg:w-[500px]  lg:h-[23.8rem] bg-contain`}
      >
